(() => {
      ...
      next() {
        ...
        do {
          if (current2 === null) {
            ...
            current2 = iterables[i][Symbol.iterator]();
          }
          step = current2.next();
          if (step.done) {
            current2 = null;
  ...
  EdgeData.prototype.attachMulti = function() {
    ...
    const target = this.target.key;
    if (this.undirected) outKey = inKey = "undirected";
    const adj2 = this.source[outKey];
    const head = adj2[target];
  ...
  function forEachAdjacency(breakable, assymetric, disconnectedNodes, graph, callback) {
    ...
    let step, sourceData, neighbor, adj2, edgeData, targetData, shouldBreak;
    while (step = iterator.next(), step.done !== true) {
      let hasEdges = false;
...
`
  ...
  var Sigma$1 = /* @__PURE__ */ (function(_TypedEventEmitter) {
    ...
    return _createClass(Sigma2, [{
      ...
      value: function createCanvasContext(id) {
        var options = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
        var canvas2 = this.createCanvas(id, options);
        var contextOptions = {
          preserveDrawingBuffer: false,
          antialias: false
        };
        this.canvasContexts[id] = canvas2.getContext("2d", contextOptions);
        return this;
      }
      /**
      ...
      value: function createWebGLContext(id) {
        var options = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : {};
        var canvas2 = (options === null || options === void 0 ? void 0 : options.canvas) || this.createCanvas(id, options);
        if (options.hidden) canvas2.remove();
        var contextOptions = _objectSpread2({
          preserveDrawingBuffer: false,
          antialias: false
        }, options);
        var context;
        context = canvas2.getContext("webgl2", contextOptions);
        if (!context) context = canvas2.getContext("webgl", contextOptions);
        if (!context) context = canvas2.getContext("experimental-webgl", contextOptions);
        var gl = context;
  ...
  // src/app.js
  var D = window.KGW_DATA;
  var byId = new Map(D.documents.map((x) => [x.id, x]));
  var nodeById = new Map(D.nodes.map((x) => [x.id, x]));
  var titleMap = /* @__PURE__ */ new Map();
  ...
  var current = D.site?.homeId || "PAGE_HOME";
  var tab = "domains";
  function findTitle(t) {
    return titleMap.get(String(t).trim().toLowerCase());
  }
  function search(q) {
    ...
    if (q) {
      arr = arr.map((n) => ({
        n,
        score: (n.title?.toLowerCase().includes(q) ? 12 : 0) + (n.title_en?.toLowerCase().includes(q) ? 9 : 0) + ((n.aliases || []).join(" ").toLowerCase().includes(q) ? 7 : 0) + (n.body?.toLowerCase().includes(q) ? 1 : 0)
  ...
  function renderSide(arr) {
    ...
    document.querySelectorAll(".side-item[data-id]").forEach((x) => {
      x.onclick = () => show(x.dataset.id, { syncSidebar: true, sidebarBehavior: "smooth" });
    });
  }
  function bindLinks() {
  ...
  function show(id, options = {}) {
    ...
    document.title = `${d.title} \xB7 ${D.site?.title || "Knowledge Atlas"}`;
  ...
  var MACRO_GROUPS = D.macros || Object.fromEntries(Object.entries(D.domains || {}).map(([id, value]) => [id, { label: value.label, color: value.color, domains: [id] }]));
  var macroByDomain = /* @__PURE__ */ new Map();
  for (const [key, value] of Object.entries(MACRO_GROUPS)) for (const d of value.domains) macroByDomain.set(d, key);
  var DOMAIN_COLORS = Object.fromEntries(Object.entries(D.domains || {}).map(([key, value]) => [key, value.color]));
  var TYPE_GROUPS = {
    concept: { label: "\uAC1C\uB150", color: "#60a5fa", types: ["ConceptOutline", "DomainConcept"] },
    theory: { label: "\uC774\uB860\xB7\uBC95\uCE59\xB7\uADFC\uC0AC", color: "#a78bfa", types: ["ModelOrTheory", "Law", "Approximation"] },
    equation: { label: "\uC218\uC2DD\xB7\uC218\uD559 \uAC1D\uCCB4", color: "#f59e0b", types: ["Equation", "MathematicalObject"] },
    method: { label: "\uC2E4\uD5D8\xB7\uACC4\uC0B0 \uBC29\uBC95", color: "#2dd4bf", types: ["ExperimentalMethod", "ComputationalMethod"] },
    quantity: { label: "\uBB3C\uB9AC\uB7C9\xB7\uAD00\uCE21\uB7C9", color: "#22c55e", types: ["ThermodynamicQuantity", "PhysicalQuantity", "TransportQuantity", "KineticQuantity", "StateFunction", "Observable"] },
    process: { label: "\uC0C1\uD0DC\xB7\uACFC\uC815\xB7\uC870\uAC74", color: "#fb7185", types: ["Process", "Condition", "ThermodynamicState", "Classification"] },
    map: { label: "\uC9C0\uB3C4\xB7\uD398\uC774\uC9C0", color: "#cbd5e1", types: ["TopicMap", "DomainMap", "GraphView", "Page", "DataStructure"] }
  };
  var typeGroupByType = /* @__PURE__ */ new Map();
  ...
  var REVIEW_GROUPS = {
    ...
    map: { label: "\uC9C0\uB3C4\xB7\uD398\uC774\uC9C0", color: "#a78bfa" },
    other: { label: "\uAE30\uD0C0", color: "#94a3b8" }
  };
  var COMMUNITY_HUES = [235, 174, 334, 42, 195, 272, 98, 18, 187, 51, 286, 166, 346, 86];
  var RELATION_STYLES = {
    hierarchy: { label: "\uAD6C\uC870\xB7\uACC4\uCE35", color: "#94a3b8", dash: [7, 5], width: 1.15 },
    dependency: { label: "\uC758\uC874\xB7\uC720\uB3C4", color: "#60a5fa", dash: [], width: 1.35 },
    causal: { label: "\uC778\uACFC\xB7\uC601\uD5A5", color: "#fb923c", dash: [], width: 1.55 },
    theory: { label: "\uC774\uB860\xB7\uD45C\uD604", color: "#a78bfa", dash: [12, 4], width: 1.3 },
    experiment: { label: "\uCE21\uC815\xB7\uBC29\uBC95", color: "#2dd4bf", dash: [], width: 1.45 },
    process: { label: "\uACFC\uC815\xB7\uBA54\uCEE4\uB2C8\uC998", color: "#fb7185", dash: [2, 5], width: 1.4 },
    association: { label: "\uC9C1\uC811 \uAC1C\uB150 \uC5F0\uACB0", color: "#64748b", dash: [], width: 0.8 },
    scaffold: { label: "\uC8FC\uC81C \uADFC\uC811(\uBC30\uCE58 \uBCF4\uC870)", color: "#475569", dash: [2, 5], width: 0.55 }
  };
  var RELATION_LABELS = {
    PART_OF: "\uBD80\uBD84/\uC0C1\uC704",
    TOPIC_PROXIMITY: "\uAC19\uC740 \uC8FC\uC81C \uC9C0\uB3C4",
    GRAPH_PROXIMITY: "\uAD50\uCC28 \uC8FC\uC81C \uC5F0\uACB0",
    DOMAIN_PROXIMITY: "\uAC19\uC740 \uC138\uBD80 \uC601\uC5ED",
    APPEARS_IN: "\uD3EC\uD568\uB428",
    APPROXIMATED_BY: "\uADFC\uC0AC\uB428",
    ESTIMATED_BY: "\uCD94\uC815\uB428",
    HAS_SUBTYPE: "\uD558\uC704 \uC720\uD615",
    ...
    OPTIMIZES: "\uCD5C\uC801\uD654",
    REDUCES: "\uAC10\uC18C",
    REDUCES_CONTRIBUTION_OF: "\uCE21\uC815 \uAE30\uC5EC \uAC10\uC18C",
    LINEARIZES_TO: "\uC120\uD615\uD654",
    RELATED_TO: "\uAD00\uB828",
    IS_A: "\uC885\uB958",
    HAS_COMPONENT: "\uAD6C\uC131",
    STATE_VARIABLE_OF: "\uC0C1\uD0DC\uBCC0\uC218",
    DEPENDS_ON: "\uC758\uC874",
    ACCOUNTS_FOR: "\uBC18\uC601",
    DERIVED_FROM: "\uC720\uB3C4",
    REQUIRES: "\uD544\uC694",
  ...
  function typeGroup(doc) {
    return typeGroupByType.get(doc?.type) || "concept";
  }
  function isMapNode(doc) {
    return ["TopicMap", "DomainMap", "GraphView", "Page"].includes(doc?.type);
  }
  var partParents = /* @__PURE__ */ new Map();
  ...
  function communityColor(key) {
    return `hsl(${COMMUNITY_HUES[hash32(key) % COMMUNITY_HUES.length]} 72% 60%)`;
  }
  var canvas = qs("#graphCanvas");
  var ctx = canvas.getContext("2d");
  var stage = qs("#graphStage");
  var sigmaContainer = qs("#sigmaContainer");
  var tooltip = qs("#tooltip");
  var storedGraph = JSON.parse(safeStorage.get(`${D.site?.slug || "knowledge-atlas"}-graph-options`) || "{}");
  var COMMUNITY_META = new Map((D.globalGraph?.communities || []).map((item) => [Number(item.id), item]));
  var GLOBAL_NODE_META = new Map((D.globalGraph?.nodes || []).map((item) => [item.id, item]));
  var GLOBAL_TOP_LABELS = new Set((D.globalGraph?.nodes || []).slice().sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, 8).map((item) => item.id));
  var GRAPH_DEFAULTS = {
    viewMode: "overview",
    layoutVariant: "linlog",
    depth: 2,
    colorMode: "community",
    labelMode: "auto",
    sizeMode: "degree",
  ...
  var G = {
    root: null,
    viewMode: storedGraph.viewMode || GRAPH_DEFAULTS.viewMode,
    layoutVariant: storedGraph.layoutVariant || GRAPH_DEFAULTS.layoutVariant,
    depth: Number(storedGraph.depth || GRAPH_DEFAULTS.depth),
    colorMode: storedGraph.colorMode || GRAPH_DEFAULTS.colorMode,
    labelMode: storedGraph.labelMode || GRAPH_DEFAULTS.labelMode,
    sizeMode: storedGraph.sizeMode || GRAPH_DEFAULTS.sizeMode,
    arrows: storedGraph.arrows ?? GRAPH_DEFAULTS.arrows,
    showMaps: storedGraph.showMaps ?? GRAPH_DEFAULTS.showMaps,
    nodeScale: Number(storedGraph.nodeScale || 100) / 100,
    distance: Number(storedGraph.distance || GRAPH_DEFAULTS.distance),
    repel: Number(storedGraph.repel || GRAPH_DEFAULTS.repel),
    nodes: [],
    links: [],
  ...
  function saveGraphOptions() {
    safeStorage.set(`${D.site?.slug || "knowledge-atlas"}-graph-options`, JSON.stringify({
      ...
      arrows: G.arrows,
      showMaps: G.showMaps,
      nodeScale: Math.round(G.nodeScale * 100),
      distance: G.distance,
      repel: G.repel
    }));
  }
  function resize() {
    const previousWidth = G.w;
    const previousHeight = G.h;
    ...
    canvas.height = Math.round(r.height * G.dpr);
    ...
    if (G.overviewFallback) {
      if (previousWidth <= 1 || previousHeight <= 1) fitOverviewCanvas();
      else drawOverviewCanvas();
    } else if (G.viewMode === "focus") {
      if (previousWidth <= 1 || previousHeight <= 1) fitLocal();
      else drawLocal();
    }
  }
  ...
  function edgePriority(edge, rootDomain) {
    ...
    return (edge.relation === "RELATED_TO" ? 1 : 5) + (other?.domain && other.domain !== rootDomain ? 2 : 0) + Math.log2((nodeById.get(edge.other)?.degree || 0) + 1) + Number(edge.confidence || 1);
  }
  function subgraph(root) {
    const rootDomain = byId.get(root)?.domain;
    const maxNodes = G.depth === 1 ? 28 : G.depth === 2 ? 62 : 90;
    const seen = /* @__PURE__ */ new Map([[root, 0]]);
    let frontier = [root];
  ...
  function buildLocalParentMap(root, levels) {
    const rootDomain = byId.get(root)?.domain;
    const parents = /* @__PURE__ */ new Map([[root, null]]);
  ...
  function localNodeMeta(id, level, index) {
    ...
    const angle = hash32(`${doc.domain}|${id}`) % 6283 / 1e3 + index * 0.09;
    const radius = level ? 95 + level * 110 + hash32(id) % 36 : 0;
    return {
      id,
      label: node.label || doc.title || id,
      domain: node.domain || doc.domain || "foundations",
      type: doc.type || node.type || "Concept",
      review: reviewGroup(doc),
      typeGroup: typeGroup(doc),
      community: topicCommunity(id),
      ...
      branchAngle: angle,
      fixed: id === G.localRoot,
      pinned: id === G.localRoot
    };
  }
  function setLocalAnchor(node, x, y, angle) {
    const jitterX = (hash32(`${node.id}|focus-x`) % 1e3 / 1e3 - 0.5) * 7;
    const jitterY = (hash32(`${node.id}|focus-y`) % 1e3 / 1e3 - 0.5) * 7;
    node.anchorX = x;
    node.anchorY = y;
  ...
  function applyDandelionLayout(root, nodes, parents) {
    ...
    for (const node of nodes) {
      ...
      node.fixed = node.id === root;
      node.pinned = node.id === root;
      if (node.parentId) {
    ...
    const startAngle = hash32(`${root}|dandelion`) % 6283 / 1e3 - Math.PI;
    ...
    for (const node of nodes) {
      ...
      const angle = hash32(`${node.id}|orphan`) % 6283 / 1e3 - Math.PI;
  ...
  function buildLocalGraph(root) {
    ...
    for (const edge of G.links) {
      edge.a.localDegree += 1;
      edge.b.localDegree += 1;
    }
  ...
  function startLocalSimulation() {
    ...
    G.running = true;
    requestAnimationFrame(stepLocal);
  }
  function stepLocal() {
    if (!G.running || G.paused || G.viewMode !== "focus") {
      G.running = false;
      return;
    }
    for (let i = 0; i < G.nodes.length; i += 1) {
      ...
      for (let j = i + 1; j < G.nodes.length; j += 1) {
        ...
        let d2 = Math.max(48, dx * dx + dy * dy);
    ...
    for (const edge of G.links) {
      ...
      const strength = treeEdge ? 42e-4 : 8e-5;
    ...
    G.alpha *= 0.98;
    drawLocal();
    if ((energy > 0.02 || G.alpha > 0.025) && G.alpha > 0.01) requestAnimationFrame(stepLocal);
  ...
  function localScreenToWorld(x, y) {
    return { x: (x - G.w / 2) / G.zoom - G.panX, y: (y - G.h / 2) / G.zoom - G.panY };
  }
  function overviewCommunityColor(id) {
    return COMMUNITY_META.get(Number(id))?.color || communityColor(`global:${id}`);
  }
  function overviewCommunityLabel(id) {
    const meta = COMMUNITY_META.get(Number(id));
    if (!meta) return "\uC8FC\uC81C \uAD70\uC9D1";
    const names = (meta.label ? [meta.label] : (meta.topNodes || []).slice(0, 2).map((item) => item.title)).filter(Boolean).join("\xB7");
  ...
  function localNodeVisible(node) {
    return !G.hiddenColors.has(nodeColorKey(node));
  }
  function localLinkVisible(edge) {
    return localNodeVisible(edge.a) && localNodeVisible(edge.b) && !G.hiddenRelations.has(edge.category);
  }
  function localNeighbors(id) {
    const result = /* @__PURE__ */ new Set();
    for (const edge of G.links) {
      if (edge.a.id === id) result.add(edge.b.id);
      if (edge.b.id === id) result.add(edge.a.id);
    }
    return result;
  }
  function localCurve(edge) {
    const a = localPos(edge.a);
    const b = localPos(edge.b);
    const dx = b.x - a.x;
    const dy = b.y - a.y;
  ...
  function drawLocal() {
    ...
    const neighbors = focus ? localNeighbors(focus.id) : null;
    for (const edge of G.links) {
      if (!localLinkVisible(edge)) continue;
      const { a, b, c } = localCurve(edge);
      const style = RELATION_STYLES[edge.category];
      const hot = !focus || focus.id === edge.a.id || focus.id === edge.b.id;
      const alpha = hot ? focus ? 0.8 : 0.28 : 0.035;
      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.strokeStyle = style.color;
      ctx.lineWidth = style.width * (focus && hot ? 1.3 : 1);
      ...
      ctx.restore();
      if (G.arrows && hot) {
        const p84 = quadraticAt(a, c, b, 0.84);
        const p89 = quadraticAt(a, c, b, 0.89);
        drawArrow(ctx, p84, p89, style.color, alpha);
      }
    }
    for (const node of [...G.nodes].sort((a, b) => (a.id === G.localRoot ? 1 : 0) - (b.id === G.localRoot ? 1 : 0))) {
      if (!localNodeVisible(node)) continue;
      const p = localPos(node);
      const radius = Math.max(2.8, localNodeRadius(node) * G.zoom * G.nodeScale);
      const color = nodeColor(node);
      const hot = !focus || focus.id === node.id || neighbors?.has(node.id);
      const depthAlpha = node.level === 0 ? 1 : node.level === 1 ? 0.88 : node.level === 2 ? 0.64 : 0.44;
      const alpha = (hot ? 1 : 0.12) * depthAlpha;
      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.fillStyle = color;
      if (node.id === G.localRoot || node === focus) {
        ctx.shadowBlur = 10;
      ...
      if (showLabel && hot) {
        ...
        ctx.font = `${node.id === G.localRoot ? 700 : 600} ${node.id === G.localRoot ? 12 : 10.5}px Pretendard, Noto Sans KR, sans-serif`;
        ctx.textAlign = "center";
        ctx.textBaseline = "top";
        let label = node.label;
        while (label.length > 5 && ctx.measureText(label).width > 160) label = `${label.slice(0, -2)}\u2026`;
        ctx.lineWidth = 3;
        ctx.strokeStyle = "rgba(5,8,12,.9)";
        ctx.strokeText(label, p.x, p.y + radius + 5);
  ...
  function localHit(x, y) {
    ...
    for (const node of G.nodes) {
      ...
      const radius = Math.max(11, localNodeRadius(node) * G.zoom * G.nodeScale + 7);
  ...
  function showTooltipNode(node, point, overview = false) {
    ...
    const extra = overview ? `<br>\uAC00\uC911 \uC5F0\uACB0 ${node.weightedDegree.toFixed(2)} \xB7 \uB9E4\uAC1C \uC911\uC2EC\uC131 ${node.betweenness.toFixed(3)}` : `<br>\uD604\uC7AC \uADF8\uB798\uD504 \uC5F0\uACB0 ${node.localDegree} \xB7 \uC804\uCCB4 \uC5F0\uACB0 ${node.degree}`;
    tooltip.innerHTML = `<b>${esc(node.label || node.title)}</b><small>${esc(colorLabel(key))} \xB7 ${esc(TYPE_GROUPS[node.typeGroup]?.label || node.type)}${extra}</small>`;
    tooltip.hidden = false;
    tooltip.style.left = `${Math.min(G.w - 290, Math.max(7, point.x + 13))}px`;
    tooltip.style.top = `${Math.min(G.h - 90, Math.max(7, point.y + 13))}px`;
  }
  function colorToHsl(color) {
    ...
    if (String(color).startsWith("#")) {
      ...
      const full = hex.length === 3 ? [...hex].map((v) => v + v).join("") : hex;
      const r = parseInt(full.slice(0, 2), 16) / 255;
      const g = parseInt(full.slice(2, 4), 16) / 255;
      const b = parseInt(full.slice(4, 6), 16) / 255;
      const max = Math.max(r, g, b);
  ...
  function computeRadialTargets(records, rootId, neighborFn, coordinateScale = 1) {
    ...
    if (!root) return { targets: /* @__PURE__ */ new Map(), distances: /* @__PURE__ */ new Map(), extent: 1 };
    ...
    const targets = /* @__PURE__ */ new Map([[rootId, { x: 0, y: 0 }]]);
    const startOffset = hash32(rootId) % 6283 / 1e3 - Math.PI;
    let extent = 0;
    for (const [layer, entries] of [...layers.entries()].sort((a, b) => a[0] - b[0])) {
      ...
      entries.forEach((entry, index) => {
        ...
        const radialJitter = (hash32(`${rootId}|${entry.node.id}`) % 1e3 / 1e3 - 0.5) * 0.045 * coordinateScale;
  ...
  function cancelLayoutAnimation() {
    if (G.layoutAnimation) cancelAnimationFrame(G.layoutAnimation);
    G.layoutAnimation = null;
  }
  function animateSigmaCoordinates(targets, duration = 860, extent = 1.15) {
    ...
    const tick = (now) => {
      ...
      G.sigmaGraph.forEachNode((id) => {
        ...
        const target = targets.get(id) || start;
  ...
  function restoreSigmaCoordinates(duration = 560) {
    ...
    G.sigmaGraph.forEachNode((id, attrs) => {
      const x = attrs.baseX ?? attrs.x;
      const y = attrs.baseY ?? attrs.y;
      targets.set(id, { x, y });
  ...
  function restoreCanvasCoordinates(duration = 560) {
    ...
    let extent = 420;
    for (const node of G.overviewNodes) {
      targets.set(node.id, { x: node.baseX, y: node.baseY });
      extent = Math.max(extent, Math.abs(node.baseX), Math.abs(node.baseY));
    }
  ...
  function applyOverviewFocus(rootId, animate = true) {
    ...
    if (G.sigmaGraph && G.sigma) {
      if (!G.overviewFocusRoot) {
        G.overviewDistances = /* @__PURE__ */ new Map();
        restoreSigmaCoordinates(animate ? 760 : 0);
        return;
      }
    ...
    if (G.overviewFallback && G.overviewNodes?.length) {
      if (!G.overviewFocusRoot) {
        G.overviewDistances = /* @__PURE__ */ new Map();
        restoreCanvasCoordinates(animate ? 760 : 0);
        return;
      }
  ...
  function overviewNodeVisible(node) {
    return !G.hiddenColors.has(nodeColorKey(node));
  }
  function overviewEdgeVisible(edge) {
    const source = G.overviewMap.get(edge.source);
    const target = G.overviewMap.get(edge.target);
  ...
  function fitOverviewCanvas(preserveFocusCenter = false, forcedExtent = null) {
    ...
    const width = Math.max(100, maxX - minX + 58);
    const height = Math.max(100, maxY - minY + 58);
    G.zoom = Math.max(0.35, Math.min(2.4, Math.min(G.w / width, G.h / height)));
    G.panX = -(minX + maxX) / 2;
    G.panY = -(minY + maxY) / 2;
    drawOverviewCanvas();
  }
  function overviewHit(x, y) {
    ...
    for (const node of G.overviewNodes) {
      ...
      const point = overviewWorldPosition(node);
  ...
  function drawOverviewCanvas() {
    ...
    ctx.setTransform(G.dpr, 0, 0, G.dpr, 0, 0);
    ctx.clearRect(0, 0, G.w, G.h);
    const focus = G.hover?.id || G.overviewFocusRoot || null;
    const neighbors = focus ? overviewNeighbors(focus) : /* @__PURE__ */ new Set();
    for (const edge of G.overviewLinks) {
      if (!overviewEdgeVisible(edge)) continue;
      const source = G.overviewMap.get(edge.source);
      const target = G.overviewMap.get(edge.target);
      const a = overviewWorldPosition(source);
      const b = overviewWorldPosition(target);
      const sourceDistance = overviewVisualDistance(edge.source, G.hover?.id || null);
      const targetDistance = overviewVisualDistance(edge.target, G.hover?.id || null);
      ...
      const alpha = hot ? edge.weak ? 0.24 : 0.56 : near ? edge.weak ? 0.1 : 0.22 : 0.045;
      ctx.save();
      ctx.globalAlpha = alpha;
      ctx.strokeStyle = "#aeb8c7";
      ctx.lineWidth = hot ? edge.weak ? 0.6 : 1 : near ? 0.58 : 0.34;
      ctx.beginPath();
      ctx.moveTo(a.x, a.y);
      ctx.lineTo(b.x, b.y);
      ctx.stroke();
    ...
    for (const node of ordered) {
      if (!overviewNodeVisible(node)) continue;
      const point = overviewWorldPosition(node);
      const adjacent = neighbors.has(node.id);
      const hot = !focus || node.id === focus || adjacent;
      ...
      ctx.save();
      ctx.globalAlpha = distance == null ? 0.96 : distance <= 1 ? 0.98 : distance === 2 ? 0.78 : 0.58;
      ctx.fillStyle = focusTone(baseColor, distance);
      if (node.id === focus) {
        ctx.shadowBlur = 11;
      ...
      if (showLabel && (hot || !focus)) {
        ...
        ctx.font = `${node.id === focus ? 700 : 600} ${node.id === focus ? 12 : 10}px Pretendard, Noto Sans KR, sans-serif`;
        ctx.textAlign = "center";
        ctx.textBaseline = "top";
        let label = node.label;
        while (label.length > 5 && ctx.measureText(label).width > 145) label = `${label.slice(0, -2)}\u2026`;
        ctx.lineWidth = 3;
        ctx.strokeStyle = "rgba(4,7,12,.94)";
        ctx.strokeText(label, point.x, point.y + radius + 4);
        ctx.fillStyle = "#eef2f7";
  ...
  function buildCanvasOverview(root) {
    G.overviewFallback = true;
    G.sigma = null;
    G.sigmaGraph = null;
    G.sigmaHovered = null;
    G.sigmaSelected = null;
    G.sigmaNeighbors = /* @__PURE__ */ new Set();
    sigmaContainer.hidden = true;
    canvas.hidden = false;
    G.root = GLOBAL_NODE_META.has(root) ? root : null;
    G.overviewNodes = (D.globalGraph?.nodes || []).map((raw) => {
      const node = globalNodeRecord(raw);
      const x = G.layoutVariant === "standard" ? raw.xStandard : raw.x;
      const y = G.layoutVariant === "standard" ? raw.yStandard : raw.y;
    ...
    G.hover = null;
    G.selected = G.root ? G.overviewMap.get(G.root) : null;
    G.zoom = 1;
    G.panX = 0;
    G.panY = 0;
    fitOverviewCanvas();
  ...
  function buildSigmaOverview(root, forceRebuild = false) {
    G.root = GLOBAL_NODE_META.has(root) ? root : null;
    if (G.sigma && !forceRebuild) {
      G.sigmaSelected = G.root;
    ...
    for (const edge of D.globalGraph?.edges || []) {
      ...
      graph.addUndirectedEdgeWithKey(`g${edgeIndex++}`, edge.source, edge.target, {
        ...
        relation: edge.dominantRelation,
        category: relationCategory(edge.dominantRelation),
        size: edge.weak ? 0.25 : 0.45 + Math.min(0.65, edge.weight * 0.38),
        color: edge.weak ? "rgba(139,149,165,.12)" : "rgba(139,149,165,.26)"
      });
    }
    G.sigmaGraph = graph;
    G.sigmaSelected = G.root;
    ...
    try {
      ...
      G.sigma = new Sigma(graph, sigmaContainer, {
        ...
        labelWeight: "600",
        labelColor: { color: "#e8edf5" },
        labelRenderedSizeThreshold: 7.5,
        labelDensity: 0.08,
        labelGridCellSize: 130,
        defaultNodeColor: "#60a5fa",
        defaultEdgeColor: "rgba(139,149,165,.2)",
        minCameraRatio: 0.12,
        maxCameraRatio: 8,
        nodeReducer: (id, data) => {
          const node = globalNodeRecord(data);
          const hidden = G.hiddenColors.has(nodeColorKey(node));
        ...
        edgeReducer: (id, data) => {
          ...
          const sourceData = globalNodeRecord(graph.getNodeAttributes(source));
          const targetData = globalNodeRecord(graph.getNodeAttributes(target));
          ...
          const interactionFocus = G.sigmaHovered || G.sigmaSelected || G.overviewFocusRoot;
          const sourceDistance = overviewVisualDistance(source, G.sigmaHovered || null);
          const targetDistance = overviewVisualDistance(target, G.sigmaHovered || null);
      ...
      G.sigma.on("enterNode", ({ node, event }) => {
        G.sigmaHovered = node;
        G.sigmaNeighbors = new Set(graph.neighbors(node));
        const raw = globalNodeRecord(graph.getNodeAttributes(node));
        showTooltipNode(raw, { x: event.x, y: event.y }, true);
        G.sigma.refresh();
      });
      G.sigma.on("leaveNode", () => {
        G.sigmaHovered = null;
        G.sigmaNeighbors = G.sigmaSelected ? new Set(graph.neighbors(G.sigmaSelected)) : /* @__PURE__ */ new Set();
        tooltip.hidden = true;
        G.sigma.refresh();
      });
      G.sigma.on("clickNode", ({ node }) => {
        G.sigmaSelected = node;
        show(node, { syncSidebar: true, sidebarBehavior: "smooth" });
      });
      G.sigma.on("clickStage", () => {
        G.root = null;
        G.sigmaSelected = null;
        G.sigmaNeighbors = /* @__PURE__ */ new Set();
        applyOverviewFocus(null, true);
        updateGraphChrome();
        G.sigma.refresh();
      });
      sigmaContainer.hidden = false;
      canvas.hidden = true;
      updateSigmaFocus();
      if (G.root) applyOverviewFocus(G.root, false);
    } catch (error) {
      ...
      G.sigma = null;
      G.sigmaGraph = null;
      buildCanvasOverview(root);
    }
  }
  function updateSigmaFocus() {
    if (!G.sigmaGraph || !G.sigma) return;
    const focus = G.sigmaHovered || G.sigmaSelected || G.overviewFocusRoot;
    G.sigmaNeighbors = focus && G.sigmaGraph.hasNode(focus) ? new Set(G.sigmaGraph.neighbors(focus)) : /* @__PURE__ */ new Set();
    G.sigma.refresh();
  }
  function fitLocal() {
    ...
    const width = Math.max(100, maxX - minX + 110);
    const height = Math.max(100, maxY - minY + 110);
    G.zoom = Math.max(0.34, Math.min(1.8, Math.min(G.w / width, G.h / height)));
    G.panX = -(minX + maxX) / 2;
    G.panY = -(minY + maxY) / 2;
    drawLocal();
  }
  function updateGraphChrome() {
    ...
    qs("#graphSubtitle").textContent = overview ? G.overviewFocusRoot ? `\uC120\uD0DD\uD55C \uAC1C\uB150\uC744 \uC911\uC2EC\uC73C\uB85C \uC804\uCCB4 \uC9C0\uB3C4\uB97C \uC7AC\uBC30\uCE58\uD588\uC2B5\uB2C8\uB2E4. \uC5F0\uACB0 \uAC70\uB9AC\uAC00 \uAC00\uAE4C\uC6B4 \uAC1C\uB150\uC77C\uC218\uB85D \uC911\uC2EC \uAC00\uAE4C\uC774\uC5D0 \uB193\uC774\uBA70, \uBE44\uD65C\uC131 \uB178\uB4DC\uB3C4 \uC6D0\uB798 \uC0C9\uC870\uB97C \uC720\uC9C0\uD569\uB2C8\uB2E4.` : `\uC9C1\uC811 \uAD00\uACC4\uC640 \uC8FC\uC81C \uADFC\uC811 \uAD00\uACC4\uB85C \uC790\uB3D9 \uD0D0\uC9C0\uD55C ${D.graphMetrics?.selectedCommunityCount || 0}\uAC1C \uCEE4\uBBA4\uB2C8\uD2F0\uC785\uB2C8\uB2E4. ForceAtlas2 ${G.layoutVariant === "linlog" ? "LinLog" : "\uAE30\uBCF8"} \uC88C\uD45C\uB97C \uBE4C\uB4DC \uC2DC \uACE0\uC815\uD588\uC2B5\uB2C8\uB2E4.` : "\uD604\uC7AC \uB178\uD2B8\uB97C \uC911\uC2EC\uC73C\uB85C 1\u20133\uB2E8\uACC4 \uAD00\uACC4\uB97C \uD0D0\uC0C9\uD569\uB2C8\uB2E4. \uAD00\uACC4 \uC720\uD615\uACFC \uBC29\uD5A5\uC740 \uD3EC\uCEE4\uC2A4 \uADF8\uB798\uD504\uC5D0\uC11C \uD655\uC778\uD569\uB2C8\uB2E4.";
    if (overview) {
      sigmaContainer.hidden = G.overviewFallback;
      canvas.hidden = !G.overviewFallback;
    } else {
      sigmaContainer.hidden = true;
      canvas.hidden = false;
    }
    qs("#pause").disabled = overview;
    qs("#pause").title = overview ? "\uC804\uCCB4 \uC9C0\uB3C4 \uC88C\uD45C\uB294 \uBE4C\uB4DC \uC2DC \uACE0\uC815\uB429\uB2C8\uB2E4." : "\uBC30\uCE58 \uC815\uC9C0/\uC7AC\uAC1C";
  ...
  function updateLegends() {
    ...
    document.querySelectorAll("[data-color-key]").forEach((element) => {
      element.onclick = () => {
        const key = Number.isNaN(Number(element.dataset.colorKey)) ? element.dataset.colorKey : Number(element.dataset.colorKey);
        if (G.hiddenColors.has(key)) G.hiddenColors.delete(key);
        else G.hiddenColors.add(key);
        updateLegends();
        refreshGraphDisplay();
      };
    });
    qs("#typeLegend").innerHTML = G.viewMode === "overview" ? `<span class="legend-chip static"><i class="size-dot small"></i>\uC8FC\uBCC0 \uAC1C\uB150</span><span class="legend-chip static"><i class="size-dot large"></i>\uC911\uC2EC\uC131 \uB192\uC740 \uD5C8\uBE0C</span><span class="legend-chip static"><i class="opacity-dot near"></i>\uC120\uD0DD \uC774\uC6C3</span><span class="legend-chip static"><i class="opacity-dot far"></i>\uB098\uBA38\uC9C0 \uC57D\uD654</span>` : `<span class="legend-chip static"><i class="size-dot small"></i>\uC5F0\uACB0 \uC801\uC74C</span><span class="legend-chip static"><i class="size-dot large"></i>\uC5F0\uACB0 \uB9CE\uC74C</span><span class="legend-chip static"><i class="opacity-dot near"></i>\uAC00\uAE4C\uC6B4 \uB178\uB4DC</span><span class="legend-chip static"><i class="opacity-dot far"></i>\uBA3C \uB178\uB4DC</span>`;
    ...
    qs("#relationLegend").innerHTML = [...relationCounts.entries()].map(([key, count]) => {
      const style = RELATION_STYLES[key];
      return `<button class="legend-chip ${G.hiddenRelations.has(key) ? "off" : ""}" data-rel-key="${key}"><i class="relation-swatch" style="color:${style.color}"></i>${esc(style.label)} ${count}</button>`;
    }).join("");
    document.querySelectorAll("[data-rel-key]").forEach((element) => {
      element.onclick = () => {
        const key = element.dataset.relKey;
        if (G.hiddenRelations.has(key)) G.hiddenRelations.delete(key);
        else G.hiddenRelations.add(key);
        updateLegends();
        refreshGraphDisplay();
      };
    });
    if (G.viewMode === "overview") {
      qs("#graphStats").textContent = `\uAC1C\uB150 \uB178\uB4DC ${D.graphMetrics?.nodeCount || 0} \xB7 \uC9C1\uC811 \uAC1C\uB150 \uAD00\uACC4 ${D.graphMetrics?.directSemanticEdgeCount || 0} \xB7 \uBC30\uCE58 \uBCF4\uC870 \uC5F0\uACB0 ${D.graphMetrics?.scaffoldEdgeCount || 0} \xB7 \uCEE4\uBBA4\uB2C8\uD2F0 ${D.graphMetrics?.selectedCommunityCount || 0} \xB7 \uBAA8\uB4C8\uC131 ${(D.graphMetrics?.modularity || 0).toFixed(3)} \xB7 \uBC30\uCE58 \uBC29\uC2DD ${G.layoutVariant === "linlog" ? "LinLog" : "\uAE30\uBCF8"}`;
    } else qs("#graphStats").textContent = `\uD45C\uC2DC ${G.nodes.length} \uB178\uB4DC \xB7 ${G.links.length} \uAD00\uACC4 \xB7 \uAE4A\uC774 ${G.depth}`;
  }
  function renderEdges() {
    if (G.viewMode === "overview") {
      const top = (D.globalGraph?.nodes || []).slice().sort((a, b) => (b.score || 0) - (a.score || 0)).slice(0, 16);
      qs("#edgeList").innerHTML = top.map((node) => `<div class="edge" data-id="${node.id}" style="--edge-color:${overviewCommunityColor(node.community)}"><small>\uC911\uC2EC\uC131 ${(node.score || 0).toFixed(3)} \xB7 \uAC00\uC911 \uC5F0\uACB0 ${(node.weightedDegree || 0).toFixed(1)} \xB7 \uB9E4\uAC1C ${(node.betweenness || 0).toFixed(3)}</small><span>${esc(node.title)}</span></div>`).join("");
    } else {
      ...
      qs("#edgeList").innerHTML = rootEdges.map((edge) => {
        const style = RELATION_STYLES[relationCategory(edge.relation)];
        return `<div class="edge" data-id="${edge.other}" style="--edge-color:${style.color}"><small>${edge.dir === "out" ? "" : "\u2190 "}${esc(RELATION_LABELS[edge.relation] || edge.relation)}</small><span>${esc(nodeById.get(edge.other)?.label || edge.other)}</span></div>`;
      }).join("");
    }
    document.querySelectorAll(".edge").forEach((element) => {
      element.onclick = () => show(element.dataset.id);
    });
  }
  function updateControls() {
    ...
    qs("#layoutVariant").value = G.layoutVariant;
    qs("#depth").value = String(G.depth);
    qs("#colorMode").value = G.colorMode;
    qs("#labelMode").value = G.labelMode;
    qs("#sizeMode").value = G.sizeMode;
    qs("#arrows").checked = G.arrows;
    qs("#showMaps").checked = G.showMaps;
    qs("#nodeScale").value = String(Math.round(G.nodeScale * 100));
    qs("#distance").value = String(G.distance);
    qs("#repel").value = String(G.repel);
  }
  ...
  canvas.onpointerdown = (event) => {
    ...
    G.pointerStart = point;
    G.pointerLast = point;
    G.moved = false;
    canvas.classList.add("dragging");
    if (G.viewMode === "overview" && G.overviewFallback) {
      const node2 = overviewHit(point.x, point.y);
      G.pointerNode = node2;
      G.selected = node2 || G.selected;
      if (!node2) G.pan = true;
      drawOverviewCanvas();
      return;
    }
    ...
    if (node) {
      ...
      G.selected = node;
      node.fixed = true;
      const world = localScreenToWorld(point.x, point.y);
      node.dragOffsetX = node.x - world.x;
      node.dragOffsetY = node.y - world.y;
    } else G.pan = true;
    drawLocal();
  };
  canvas.onpointermove = (event) => {
    ...
    if (G.viewMode === "overview" && G.overviewFallback) {
      if (G.pointerStart) {
        ...
        if (G.pan) {
          G.panX += (point.x - G.pointerLast.x) / G.zoom;
          G.panY += (point.y - G.pointerLast.y) / G.zoom;
        }
        G.pointerLast = point;
        drawOverviewCanvas();
        return;
      }
      G.hover = overviewHit(point.x, point.y);
      showTooltipNode(G.hover, point, true);
      drawOverviewCanvas();
      return;
    }
    if (G.viewMode !== "focus") return;
    if (G.pointerStart) {
      ...
      if (G.drag) {
        const world = localScreenToWorld(point.x, point.y);
        G.drag.x = world.x + G.drag.dragOffsetX;
        G.drag.y = world.y + G.drag.dragOffsetY;
        G.drag.vx = 0;
        G.drag.vy = 0;
      } else if (G.pan) {
        G.panX += (point.x - G.pointerLast.x) / G.zoom;
    ...
    G.hover = localHit(point.x, point.y);
    showTooltipNode(G.hover, point, false);
    drawLocal();
  };
  function releaseCanvasPointer(event) {
    const moved = G.moved;
    const pointerNode = G.pointerNode;
    const localNode = G.drag;
    ...
    G.moved = false;
    if (G.viewMode === "overview" && G.overviewFallback) {
      if (!moved && pointerNode) {
        G.selected = pointerNode;
        show(pointerNode.id);
      } else if (!moved && !pointerNode) {
        G.root = null;
        G.selected = null;
        applyOverviewFocus(null, true);
        updateGraphChrome();
    ...
    if (localNode) {
      if (event.shiftKey) {
        ...
        localNode.pinned = true;
      } else if (localNode.id !== G.localRoot) {
        localNode.fixed = false;
        localNode.pinned = false;
        G.alpha = 0.24;
        startLocalSimulation();
      }
  ...
  canvas.onpointerup = releaseCanvasPointer;
  canvas.onpointercancel = releaseCanvasPointer;
  ...
  canvas.onwheel = (event) => {
    ...
    const point = pointerPosition(event);
    const before = localScreenToWorld(point.x, point.y);
    G.zoom = Math.max(0.28, Math.min(5, G.zoom * Math.exp(-event.deltaY * 115e-5)));
    const after = localScreenToWorld(point.x, point.y);
    G.panX += after.x - before.x;
    G.panY += after.y - before.y;
    refreshGraphDisplay();
  };
  qs("#graphSettingsBtn").onclick = () => {
    qs("#graphSettings").hidden = !qs("#graphSettings").hidden;
  };
  ...
  qs("#layoutVariant").onchange = (event) => {
    G.layoutVariant = event.target.value;
    saveGraphOptions();
    buildGraph(G.root, true);
  };
  qs("#depth").onchange = (event) => {
    G.depth = +event.target.value;
    saveGraphOptions();
    buildGraph(G.root, true);
  };
  qs("#colorMode").onchange = (event) => {
    G.colorMode = event.target.value;
    G.hiddenColors.clear();
    saveGraphOptions();
    updateLegends();
    refreshGraphDisplay();
  };
  qs("#labelMode").onchange = (event) => {
    G.labelMode = event.target.value;
    saveGraphOptions();
    refreshGraphDisplay();
  };
  qs("#sizeMode").onchange = (event) => {
    G.sizeMode = event.target.value;
    saveGraphOptions();
    refreshGraphDisplay();
    updateLegends();
  };
  qs("#arrows").onchange = (event) => {
    G.arrows = event.target.checked;
    saveGraphOptions();
    drawLocal();
  };
  qs("#showMaps").onchange = (event) => {
    G.showMaps = event.target.checked;
    saveGraphOptions();
    if (G.viewMode === "focus") buildGraph(G.root, true);
  };
  qs("#nodeScale").oninput = (event) => {
    G.nodeScale = +event.target.value / 100;
    saveGraphOptions();
    refreshGraphDisplay();
  };
  qs("#distance").oninput = (event) => {
    G.distance = +event.target.value;
    G.alpha = 0.8;
    saveGraphOptions();
    startLocalSimulation();
  };
  qs("#repel").oninput = (event) => {
    G.repel = +event.target.value;
    G.alpha = 0.8;
    saveGraphOptions();
    startLocalSimulation();
  };
  qs("#resetFocus").onclick = () => {
    G.root = null;
    G.sigmaSelected = null;
    G.selected = null;
    applyOverviewFocus(null, true);
  ...
  qs("#defaults").onclick = () => {
    Object.assign(G, GRAPH_DEFAULTS);
    saveGraphOptions();
    buildGraph(G.root, true);
  };
  qs("#fit").onclick = () => {
    if (G.viewMode === "overview" && G.sigma) G.sigma.getCamera().animatedReset({ duration: 400 });
    else if (G.viewMode === "overview" && G.overviewFallback) fitOverviewCanvas();
    else fitLocal();
  };
  qs("#pause").onclick = () => {
    if (G.viewMode !== "focus") return;
    G.paused = !G.paused;
    qs("#pause").textContent = G.paused ? "\u25B6" : "\u2161";
    if (!G.paused) {
  ...
  document.addEventListener("pointerdown", (event) => {
    if (!qs("#graphSettings").hidden && !qs("#graphSettings").contains(event.target) && event.target.id !== "graphSettingsBtn") qs("#graphSettings").hidden = true;
  });
  document.addEventListener("keydown", (event) => {
    if (event.key.toLowerCase() === "f" && !["INPUT", "TEXTAREA", "SELECT"].includes(document.activeElement?.tagName)) qs("#fit").click();
    if (event.code === "Space" && G.viewMode === "focus" && !["INPUT", "TEXTAREA", "SELECT"].includes(document.activeElement?.tagName)) {
      event.preventDefault();
      qs("#pause").click();
    }
  });
